...
async function getClientBalanceForCurrency(ownerUserId, clientId, currencyCode) {
  try {
    ...
    const raw = Number(r.rows?.[0]?.balance);
    return Number.isFinite(raw) ? raw : null;
  } catch (e) {
    logger.warning('Client balance lookup failed', { ownerUserId, clientId, currencyCode, error: e?.message });
    return null;
  }
}

function buildBalanceLine(balance, currencyCode) {
