...
// اختبار الاتصال مباشرة عند تشغيل الخادم
(async () => {
  try {
    const r = await pool.query('select now() as now');
    console.log('✅ اختبار الاتصال نجح:', r.rows[0].now);
  } catch (e) {
    console.error('❌ فشل اختبار الاتصال بقاعدة البيانات:', e.message);
    process.exit(1);
  }
})();
...
// أحداث اختيارية
pool.on('connect', () => {
  console.log('🔌 تم إنشاء اتصال جديد مع PostgreSQL');
});
