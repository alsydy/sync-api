...
function logValidationFailure(req, message, meta = {}) {
  ...
  try {
    ...
    const rowOwnerId =
      typeof row.owner_user_id === 'string'
        ...
        : row.owner_user_id;
    if (!Number.isFinite(rowOwnerId)) {
      return { ok: false, error: ACCOUNT_OWNERSHIP_ERROR };
    }
    if (rowOwnerId !== ownerIdNum) {
      return { ok: false, error: ACCOUNT_OWNERSHIP_ERROR };
    }
    return { ok: true };
  } catch (error) {
...
async function createTransaction(req, res, next) {
  try {
    ...
    if (resolved.error) {
      logValidationFailure(req, 'createTransaction: ownerUserId mismatch', {
        ownerUserId: transactionData.ownerUserId,
        ownerFirebaseUid: transactionData.ownerFirebaseUid,
        authUserId: req.user?.userId,
        authFirebaseUid: req.user?.firebaseUid || req.user?.firebase_uid,
    ...
    if (!ownerUserId) {
      logValidationFailure(req, 'createTransaction: missing ownerUserId', {
        ownerUserId: transactionData.ownerUserId,
        ownerFirebaseUid: transactionData.ownerFirebaseUid
      });
      return res.status(400).json({
    ...
    if (!accountId && transactionData.accountFirestoreId === 'shared-main-account-v1') {
      logValidationFailure(req, 'createTransaction: shared account not allowed', {
        accountFirestoreId: transactionData.accountFirestoreId
      });
      return res.status(400).json({ success: false, error: SHARED_ACCOUNT_ERROR });
    }
    if (!accountId) {
      logValidationFailure(req, 'createTransaction: missing accountId', {
    ...
    if (!accountCheck.ok) {
      logValidationFailure(req, 'createTransaction: account ownership check failed', {
        accountId,
        ownerUserId,
        accountFirestoreId: transactionData.accountFirestoreId,
...
async function syncTransaction(req, res, next) {
  try {
    ...
    if (existing.rows.length > 0) {
      ...
      if (resolved.error) {
        logValidationFailure(req, 'syncTransaction(update): ownerUserId mismatch', {
          ownerUserId: transactionData.ownerUserId,
          ownerFirebaseUid: transactionData.ownerFirebaseUid,
          authUserId: req.user?.userId,
          authFirebaseUid: req.user?.firebaseUid || req.user?.firebase_uid,
      ...
      if (!ownerUserId) {
        logValidationFailure(req, 'syncTransaction(update): missing ownerUserId', {
          ownerUserId: transactionData.ownerUserId,
          ownerFirebaseUid: transactionData.ownerFirebaseUid
        });
      ...
      if (!accountId) {
        logValidationFailure(req, 'syncTransaction(update): missing accountId', {
          accountId: transactionData.accountId,
          accountFirestoreId: transactionData.accountFirestoreId,
          ownerUserId
        });
      ...
      const accountCheck = await validateAccountForOwner(accountId, ownerUserId);
      if (!accountCheck.ok) {
        logValidationFailure(req, 'syncTransaction(update): account ownership check failed', {
          accountId,
          ownerUserId,
          accountFirestoreId: transactionData.accountFirestoreId,
          error: accountCheck.error
      ...
      if (resolved.error) {
        logValidationFailure(req, 'syncTransaction(insert): ownerUserId mismatch', {
          ownerUserId: transactionData.ownerUserId,
          ownerFirebaseUid: transactionData.ownerFirebaseUid,
          authUserId: req.user?.userId,
          authFirebaseUid: req.user?.firebaseUid || req.user?.firebase_uid,
      ...
      if (!accountId && transactionData.accountFirestoreId === 'shared-main-account-v1') {
        logValidationFailure(req, 'syncTransaction(insert): shared account not allowed', {
          accountFirestoreId: transactionData.accountFirestoreId
        });
        return res.status(400).json({ success: false, error: SHARED_ACCOUNT_ERROR });
      }
      ...
      if (!accountId) {
        logValidationFailure(req, 'syncTransaction(insert): missing accountId', {
          accountId: transactionData.accountId,
          accountFirestoreId: transactionData.accountFirestoreId,
          ownerUserId
        });
      ...
      const accountCheck = await validateAccountForOwner(accountId, ownerUserId);
      if (!accountCheck.ok) {
        logValidationFailure(req, 'syncTransaction(insert): account ownership check failed', {
          accountId,
          ownerUserId,
          accountFirestoreId: transactionData.accountFirestoreId,
          error: accountCheck.error
